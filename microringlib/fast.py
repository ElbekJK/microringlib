...
def resonance_metrics_fast(
    ...
    **kwargs,
    ...
    demos and ignored if not needed.
    """
...
def compute_resonance_metrics_fast(wavelengths, power, target_wavelength=None, **kwargs) -> dict:
...
def compute_peak_metrics_fast(wavelengths, power, target_wavelength=None, **kwargs) -> dict:
...
def monte_carlo_resonance_formula_fast(
    ...
    *,
    ...
    wavelength0: float | None = None,
    radius: float = 10e-6,
    ng: float = 4.2,
    sigma_radius: float = 5e-9,
    sigma_ng: float = 0.02,
    n_eff: float = 3.476,
    sigma_n_eff: float = 1e-4,
    ...
    """
    if n_eff_samples is None or radius_samples is None:
        if n_samples is None:
            n_samples = 1000

        if wavelength0 is None and target_wavelength is None:
            wavelength0 = 1550e-9

        if target_wavelength is None:
            target_wavelength = float(wavelength0)

        rng = np.random.default_rng(random_seed)
        radius_samples = float(radius) + rng.normal(0.0, float(sigma_radius), int(n_samples))
        n_eff_samples = float(n_eff) + rng.normal(0.0, float(sigma_n_eff), int(n_samples))
    ...
    if target_wavelength is None:
        if wavelength0 is None:
            raise ValueError("target_wavelength or wavelength0 must be provided")
        target_wavelength = wavelength0

    if n.shape != radius_arr.shape:
    ...
    circumference = 2.0 * np.pi * radius_arr
    ...
    out = {
        ...
        "wavelength_samples": resonance_wavelength,
        "mode_number": mode_number,
        "radius_samples": radius_arr,
        "n_eff_samples": n,
        "mean": float(np.mean(resonance_wavelength)),
        "std": float(np.std(resonance_wavelength)),
        "mean_nm": float(np.mean(resonance_wavelength) * 1e9),
        "std_nm": float(np.std(resonance_wavelength) * 1e9),
    }
