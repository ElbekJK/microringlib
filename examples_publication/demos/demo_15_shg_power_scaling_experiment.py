...
mode = try_fde_sweep(wl, wg['width_nm'] * 1e-9, wg['thickness_nm'] * 1e-9, float(mat['core']['constant_n']), float(mat['cladding']['constant_n']), float(mat['core']['loss_db_cm']), max_points=int(__import__("os").environ.get("MICRORINGLIB_DEMO15_FDE_POINTS","41")))
...
shg_display_scale = 2.5e-18  # arbitrary display calibration, not max-normalization
...
rolloff_onset_mw = float(pump_mw[np.argmin(np.abs(rolloff-0.95))])
shg_rate_proxy_au = ideal * rolloff * shg_display_scale
shg_rate_kcps = shg_rate_proxy_au
efficiency_kcps_per_mW2 = shg_rate_kcps / np.maximum(pump_mw**2, 1e-12)
...
rows_power = [
    {
        ...
        'shg_rate_proxy_arb': float(sn),
        'shg_rate_kcps': float(sr),
        'efficiency_kcps_per_mW2': float(ne),
        'ideal_no_rolloff_proxy_arb': float(ii * shg_display_scale),
    }
    for p, tr, dd, sn, sr, ne, ii in zip(pump_mw, thermal_rise, detuning_pm, shg_rate_proxy_au, shg_rate_kcps, efficiency_kcps_per_mW2, ideal)
]
rows_temp = [
    {
        ...
        'relative_efficiency_kcps': float(rr),
    }
    for T, dd, rr in zip(T_C, fixed_detuning_pm, fixed_rate_kcps)
]

p1 = save_csv(ROOT, 'demo_15_shg_power_scaling.csv', rows_power)
p2 = save_csv(ROOT, 'demo_15_shg_efficiency_vs_temperature.csv', rows_temp)
p3 = save_json(ROOT, 'demo_15_shg_fit_summary.json', {
...
plt.plot(pump_mw, shg_display_scale * ideal, lw=1.4, ls='--', label='ideal quadratic trend')
plt.xlabel('Pump power (mW)')
plt.ylabel('SHG rate proxy (kcps)')
plt.title('SHG power-scaling experiment proxy')
plt.legend(fontsize=8)
p4 = savefig(ROOT, 'demo_15_shg_power_scaling.png')
...
plt.legend(fontsize=8)
p5 = savefig(ROOT, 'demo_15_loglog_slope_fit.png')
...
plt.legend(fontsize=8)
p6 = savefig(ROOT, 'demo_15_shg_efficiency_vs_temperature.png')

plt.figure(figsize=(7.2, 4.2))
plt.plot(pump_mw, efficiency_kcps_per_mW2, lw=2.2)
plt.xlabel('Pump power (mW)')
plt.ylabel('SHG efficiency proxy (kcps/mW²)')
plt.title('Absolute SHG efficiency proxy / P²')
p7 = savefig(ROOT, 'demo_15_shg_efficiency_per_power2.png')

p8 = save_markdown(ROOT, 'demo_15_report.md', 'SHG power-scaling experimental proxy', {
    'Highlights': {
...
print('=== Publication demo 15: SHG power-scaling experiment ===')
