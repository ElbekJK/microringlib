...
p1=save_csv(ROOT,'demo_23_sic_thermal_locking_trace.csv',rows); p2=save_json(ROOT,'demo_23_sic_thermal_locking_summary.json',{'material_backend':stack.material_backend,'fde_backend':stack.mode.backend,'fdtd_backend':fdtd_backend,'tmm_component':'ring_allpass_field monitor curve','open_loop_rms_detuning_pm':float(np.sqrt(np.mean(det_open**2))),'closed_loop_rms_detuning_pm':float(np.sqrt(np.mean(det_closed**2))),'heater_power_mean_mW':float(np.mean(heater)),'tuning_pm_per_mW_proxy':float(tuning_pm_per_mw),'open_closed_loop_definition':'open loop is ambient drift with no heater; closed loop subtracts heater tuning and is the residual detuning around the lock target'})
plt.figure(figsize=(7.6,4.2)); plt.plot(time_ms,det_open,label='open loop: no heater correction',lw=2); plt.plot(time_ms,det_closed,label='closed loop: feedback residual',lw=2); plt.axhline(0,ls='--',lw=1,label='lock target'); plt.xlabel('Time (ms)'); plt.ylabel('Resonance detuning from target (pm)'); plt.title('4H-SiC thermal locking: open drift vs closed-loop residual'); plt.legend(fontsize=8); p3=savefig(ROOT,'demo_23_sic_locking_detuning.png')
plt.figure(figsize=(7.6,4.2)); plt.plot(time_ms,heater); plt.xlabel('Time (ms)'); plt.ylabel('Heater power (mW)'); plt.title('Feedback heater actuation'); p4=savefig(ROOT,'demo_23_sic_locking_heater_power.png')
plt.figure(figsize=(7.6,4.2)); plt.plot(time_ms,monitor_open,label='open loop'); plt.plot(time_ms,monitor_closed,label='closed loop'); plt.xlabel('Time (ms)'); plt.ylabel('Monitor power'); plt.title('FDTD-calibrated monitor signal'); plt.legend(); p5=savefig(ROOT,'demo_23_sic_locking_monitor.png')
p6=save_markdown(ROOT,'demo_23_report.md','4H-SiC thermal locking numerical precursor',{'Backends':{'materials':stack.material_backend,'FDE':stack.mode.backend,'FDTD':fdtd_backend,'TMM':'microringlib.tmm monitor ring'},'Locking metrics':{'open-loop RMS detuning':f'{np.sqrt(np.mean(det_open**2)):.3f} pm','closed-loop RMS detuning':f'{np.sqrt(np.mean(det_closed**2)):.3f} pm'},'Experiment story':'Thermo-optic material trace and FDE produce a TMM monitor curve; an FDTD hook calibrates monitor power; feedback closes the loop.'})
print('=== Publication demo 23: 4H-SiC thermal locking experiment ==='); print(f'FDE={stack.mode.backend}; FDTD={fdtd_backend}; closed-loop RMS={np.sqrt(np.mean(det_closed**2)):.3f} pm'); print('Status: PASS'); print_artifacts([p1,p2,p3,p4,p5,p6])
