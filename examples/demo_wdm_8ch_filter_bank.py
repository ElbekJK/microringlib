#!/usr/bin/env python3
"""
8-channel WDM ring filter bank demo with monotonic channel targeting.

This version is publication-friendly:
- targets specific channel center wavelengths,
- computes ring radii from a fixed azimuthal mode number,
- searches for each resonance in a narrow local wavelength window,
- avoids accidental resonance-order hopping,
- reports target/actual channel centers, spacing, Q, finesse estimate, and passivity.
...
def main():
    ...
    # ------------------------------------------------------------------
    # Desired WDM channel plan
    # ------------------------------------------------------------------
    channel_ids = np.arange(1, 9)
    target_centers_nm = 1544.0 + (channel_ids - 1) * 1.60
    ...
    # Compute radii so the same azimuthal order m0 hits the target centers.
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("wdm_8ch_filter_bank.png", dpi=250)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("wdm_channel_centers.png", dpi=250)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("wdm_channel_spacing.png", dpi=250)
